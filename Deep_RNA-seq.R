...
ttn<-read_delim("Deep_Seq_headers.txt", delim = " ", col_names = FALSE)

colnames(ttn)<-c("target_id","gene")

# Custom filter function to include more targets
#no_filter <- function(x) TRUE

# Re-run sleuth_prep with the relaxed filter
